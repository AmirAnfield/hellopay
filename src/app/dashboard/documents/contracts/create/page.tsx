...
export default function CreateContractPage() {
  ...
  const PartiesContractantesStep = ({ data, salarieData, companies, employees, onChange, onChangeSalarie }) => {
    return (
      <div className="space-y-8">
        <div className="bg-white rounded-lg shadow-sm border p-6">
          <h3 className="text-lg font-medium mb-4">Sélection de l'entreprise</h3>
                  ...
                  if (companyId) {
                    ...
                    if (company) {
                      ...
                      toast({
                        ...
                        description: `Les informations de ${company.raisonSociale} ont été chargées.`,
                        variant: "default",
                      });
                    }
                  }
                ...
                {companies && companies.length > 0 ? (
                  companies.map(company => (
                    <option key={company.id} value={company.id}>
                      {company.raisonSociale} ({company.formeJuridique}) - {company.ville}
                    </option>
                  ))
                ) : (
          ...
          {data.raisonSociale && (
            <div className="mt-4 p-4 bg-blue-50 rounded-md border border-blue-100">
              <h4 className="text-sm font-medium mb-2">Entreprise sélectionnée</h4>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                <div><span className="text-gray-500">Nom :</span> {data.raisonSociale} ({data.formeJuridique})</div>
                <div><span className="text-gray-500">SIRET :</span> {data.siret}</div>
                <div><span className="text-gray-500">Adresse :</span> {data.adresse}, {data.codePostal} {data.ville}</div>
                <div><span className="text-gray-500">Représentant :</span> {data.representant} - {data.fonction}</div>
                <div><span className="text-gray-500">Convention :</span> {data.conventionCollective || "Non renseignée"}</div>
                <div><span className="text-gray-500">Caisse de retraite :</span> {data.caisseRetraite || "Non renseignée"}</div>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="bg-white rounded-lg shadow-sm border p-6">
          <h3 className="text-lg font-medium mb-4">Sélection du salarié</h3>
          <div className="flex items-center space-x-3">
                  ...
                  if (employeeId) {
                    ...
                    if (employee) {
                      ...
                      toast({
                        ...
                        description: `Les informations de ${employee.prenom} ${employee.nom} ont été chargées.`,
                        variant: "default",
                      });
                    }
                  }
          ...
          {salarieData.nom && (
            <div className="mt-4 p-4 bg-blue-50 rounded-md border border-blue-100">
              <h4 className="text-sm font-medium mb-2">Salarié sélectionné</h4>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                <div><span className="text-gray-500">Nom complet :</span> {salarieData.civilite} {salarieData.prenom} {salarieData.nom}</div>
                <div><span className="text-gray-500">Né(e) le :</span> {salarieData.dateNaissance} à {salarieData.lieuNaissance}</div>
                <div><span className="text-gray-500">Adresse :</span> {salarieData.adresse}, {salarieData.codePostal} {salarieData.ville}</div>
                <div><span className="text-gray-500">Nationalité :</span> {salarieData.nationalite || "Non renseignée"}</div>
                <div><span className="text-gray-500">N° Sécurité sociale :</span> {salarieData.numeroSecuriteSociale || "Non renseigné"}</div>
              </div>
            </div>
          )}
        </div>
